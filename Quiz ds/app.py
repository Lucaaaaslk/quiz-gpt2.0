...
def chatbot():
    ...
    if contador == 0:
        mensagem_inicial = "Escreva o tema do quiz:"
        lista_mensagens.append({"role": "assistant", "content": mensagem_inicial})
    ...
    if request.method == 'POST':
        ...
        if contador >= 6:
            mensagem_final = "Quiz: O quiz foi finalizado."
            return render_template('index.html', response_gpt=lista_mensagens, final_message=mensagem_final)

        proximo_prompt = "Escreva sua resposta (ou escolha uma alternativa, espere 20 segundos para cada interação):"
        lista_mensagens.append({"role": "assistant", "content": proximo_prompt})

        lista_mensagens_com_quiz = [{"role": "Quiz", "content": msg["content"]} for msg in lista_mensagens]
        return render_template('index.html', response_gpt=lista_mensagens_com_quiz)
